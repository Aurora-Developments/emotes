...
--#region ptfx
RegisterNetEvent("emotes:ptfx:sync", function(asset, name, offset, rot, bone, scale, color)
    if type(asset) ~= "string" or type(name) ~= "string" or type(offset) ~= "vector3" or type(rot) ~= "vector3" then
        print("[emotes] ptfx:sync: invalid arguments for source:", source)
        return
    end
...
RegisterNetEvent("emotes:ptfx:syncProp", function(propNet)
    local srcPlayerState = Player(source).state
    if propNet then
...
local function addKeybindEventHandlers()
    RegisterNetEvent("emote:ServerKeybindExist", function()
        local src = source
        ...
        MySQL.query('SELECT * FROM dpkeybinds WHERE `id`=@id;', { id = srcid }, function(dpkeybinds)
            if dpkeybinds[1] then
                TriggerClientEvent("emote:ClientKeybindExist", src, true)
            else
                TriggerClientEvent("emote:ClientKeybindExist", src, false)
            end
        end)
    end)
    ...
    RegisterNetEvent("emote:ServerKeybindCreate", function()
        local src = source
        local srcid = GetPlayerIdentifier(src)
        MySQL.insert('INSERT INTO dpkeybinds (`id`, `keybind1`, `emote1`, `keybind2`, `emote2`, `keybind3`, `emote3`, `keybind4`, `emote4`, `keybind5`, `emote5`, `keybind6`, `emote6`) VALUES (@id, @keybind1, @emote1, @keybind2, @emote2, @keybind3, @emote3, @keybind4, @emote4, @keybind5, @emote5, @keybind6, @emote6);'
            ...
            function(created) print("[emote] ^2" .. GetPlayerName(src) .. "^7 got created!")
                TriggerClientEvent("emote:ClientKeybindGet"
                    , src, "num4", "", "num5", "", "num6", "", "num7", "", "num8", "", "num8", "")
            end)
    end)

    RegisterNetEvent("emote:ServerKeybindGrab", function()
        local src = source
        local srcid = GetPlayerIdentifier(src)
        MySQL.query('SELECT keybind1, emote1, keybind2, emote2, keybind3, emote3, keybind4, emote4, keybind5, emote5, keybind6, emote6 FROM `dpkeybinds` WHERE `id` = @id'
            ...
            if kb[1].keybind1 ~= nil then
                TriggerClientEvent("emote:ClientKeybindGet", src, kb[1].keybind1, kb[1].emote1, kb[1].keybind2, kb[1].emote2
                    , kb[1].keybind3, kb[1].emote3, kb[1].keybind4, kb[1].emote4, kb[1].keybind5, kb[1].emote5,
                    kb[1].keybind6, kb[1].emote6)
            else
                TriggerClientEvent("emote:ClientKeybindGet", src, "num4", "", "num5", "", "num6", "", "num7", "", "num8", ""
                    , "num8", "")
            end
        end)
    end)

    RegisterNetEvent("emote:ServerKeybindUpdate", function(key, emote)
        local src = source
        ...
        if chosenk == "keybind1" then
            MySQL.update("UPDATE dpkeybinds SET emote1=@emote WHERE id=@id", { id = myid, emote = emote },
                function() TriggerClientEvent("emote:ClientKeybindGetOne", src, key, emote) end)
...
if Config.SqlKeybinding and MySQL then
    MySQL.update(
        ...
        if success then
            ...
            print("[emote] ^3Error connecting to DB^7")
        end
    end)
else
    print("[emote] ^3Sql Keybinding^7 is turned ^1off^7, if you want to enable /emotebind, set ^3SqlKeybinding = ^2true^7 in config.lua and uncomment oxmysql lines in fxmanifest.lua.")
end
...
local function ExtractEmoteProps(format)
    ...
    local f, err = load(animationFile .. " return emote")
    ...
    -- emote.PropEmotes
    -- emote.Shared (most likely all props mentioned in here is used in PropEmotes, so I don't check it)
    for _, value in pairs(emote.PropEmotes) do
